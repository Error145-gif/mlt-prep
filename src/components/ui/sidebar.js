...
function SidebarProvider(_a) {
    ...
    return (<SidebarContext.Provider value={contextValue}>
      <tooltip_1.TooltipProvider delayDuration={0}>
        <div data-slot="sidebar-wrapper" style={__assign({ "--sidebar-width": SIDEBAR_WIDTH, "--sidebar-width-icon": SIDEBAR_WIDTH_ICON }, style)} className={(0, utils_1.cn)("group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-svh w-full", className)} {...props}>
          {children}
        </div>
...
function SidebarTrigger(_a) {
    ...
    return (<button_1.Button data-sidebar="trigger" data-slot="sidebar-trigger" variant="ghost" size="icon" className={(0, utils_1.cn)("size-7", className)} onClick={function (event) {
...
function SidebarMenuButton(_a) {
    ...
    var button = (<Comp data-slot="sidebar-menu-button" data-sidebar="menu-button" data-size={size} data-active={isActive} className={(0, utils_1.cn)(sidebarMenuButtonVariants({ variant: variant, size: size }), className)} {...props}/>);
    if (!tooltip) {
        return button;
    }
