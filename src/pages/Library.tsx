...
import { Lock, BookOpen, Search, X } from "lucide-react";
...
export default function Library() {
  ...
  const handleOpenPDF = (pdfUrl: string) => {
    if (!hasAccess) {
      toast.error("Library access is available only with the Yearly Plan");
      return;
    }
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        {!hasAccess && (
          <motion.div
            ...
            className="mb-6"
          >
            <Card className="glass-card border-yellow-400/50 bg-yellow-500/20">
              <CardContent className="p-4 md:p-6">
                <div className="flex flex-col md:flex-row items-center gap-4">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-white/90 text-sm md:text-base">
                      Upgrade to the Yearly Plan to unlock all PDFs and study materials
                    </p>
                  </div>
                  <Button
          ...
          >
            ...
            {displayContent.length === 0 ? (
              ...
              displayContent.map((pdf: any) => (
                <Card
                  key={pdf._id}
                  className="glass-card border-white/30 bg-white/20 hover:bg-white/30 transition-all"
                >
