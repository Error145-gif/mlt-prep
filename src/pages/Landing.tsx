...
export default function Landing() {
  ...
  // Direct Download Link (Corrected)
  ...
  const features = [
    { icon: Brain, title: "AI-Powered MCQs", description: "Practice with AI-generated questions tailored to MLT Exam patterns" },
    { icon: BookOpen, title: "Medical Lab PYQs", description: "Access comprehensive Medical Lab Previous Year Questions" },
    { icon: Award, title: "MLT Mock Tests", description: "Take full-length tests to simulate real govt exam conditions" },
    { icon: TrendingUp, title: "Track Progress", description: "Monitor your performance with detailed analytics and rankings" },
  ];

  return (
    <div className="min-h-screen relative overflow-x-hidden">
      {/* --- PURANA DESIGN START (Background & Icons) --- */}
      <div 
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {isAuthenticated ? (
              <>
                ...
                <Button onClick={() => signOut()} variant="outline" className="bg-white/10 text-white border-white/30 hidden sm:flex">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-20 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card border border-white/30 bg-white/10 text-white mb-4">
            <Sparkles className="h-4 w-4 text-yellow-300" />
            <span className="text-sm">AI-Powered Exam Preparation</span>
          </div>
          
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold text-white tracking-tight drop-shadow-md">
            Prep with <span className="bg-gradient-to-r from-yellow-300 via-orange-400 to-yellow-500 bg-clip-text text-transparent">AI Mock Tests & PYQs</span>
          </h1>

          {/* Engaging Direct Download Card */}
          <motion.div
          ...
          >
            <div className="mb-4 text-center">
              <span className="bg-yellow-400 text-blue-900 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter">Recommended for Students</span>
              <h3 className="text-2xl font-black text-white mt-2">Download MLTprep App</h3>
            </div>
            <Button 
            ...
            <p className="mt-4 text-white/80 text-xs font-medium tracking-widest flex items-center justify-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-400" /> NO PREVIEW • CLICK TO DOWNLOAD
            </p>
          </motion.div>
          ...
          <div className="flex justify-center gap-8 py-4">
             ...
             <div className="text-center"><div className="text-3xl font-bold text-white">95%</div><div className="text-sm text-white/70">Success</div></div>
          </div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 py-20">
        {features.map((f, i) => (
          <div key={i} className="glass-card p-6 rounded-2xl border border-white/20 bg-white/10 text-white backdrop-blur-md hover:bg-white/20 transition-all">
      ...
      <footer className="relative z-10 text-center py-10 text-white/50 border-t border-white/10">
        <p>© 2026 MLTprep.online - Empowering Future Lab Techs</p>
      </footer>
    </div>
  );
}
